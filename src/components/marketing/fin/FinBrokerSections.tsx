...
import {
  FIN_PLATFORM_STATS,
  FIN_PREMIUM_FEATURES,
  FIN_PRODUCTS,
  FIN_SECURITY_FEATURES,
  type FinMarketPair,
} from "@/constants/fin-markets";
...
const PRODUCT_ICONS = [LineChart, Bot, Copy, TrendingUp] as const;
...
export function FinHero({ pairs }: { pairs: FinMarketPair[] }) {
  return (
...
export function FinLiveTicker({ pairs }: { pairs: FinMarketPair[] }) {
    ...
    <FinStaggerItem>
      <FinHoverLift className="fin-card overflow-hidden">
        <div className="flex items-center justify-between border-b border-border px-5 py-3">
          ...
          <Link to="/world-economy" className="text-xs font-medium text-text-tertiary hover:text-text-primary">
            Full market view →
          </Link>
        </div>
        <motion.div
      ...
      <FinScrollStagger>
        <FinStaggerItem>
          <div className="mb-4 flex items-end justify-between gap-4 px-1">
            ...
            <Link to="/services" className="hidden text-sm font-medium text-text-tertiary hover:text-text-primary sm:block">
              All services →
            </Link>
          </div>
        </FinStaggerItem>
        <div className="grid gap-3 sm:grid-cols-2">
          {FIN_PRODUCTS.map((product, i) => {
      ...
      <FinScrollStagger>
        <FinStaggerItem>
          <div className="mb-4 px-1">
            <p className="fin-section-label">Why {BRAND.shortName}</p>
            <h2 className="mt-2 text-xl font-bold text-text-primary sm:text-2xl">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {FIN_PREMIUM_FEATURES.map((feature) => (
        ...
        <FinStaggerItem>
          <div className="fin-card p-5 sm:p-6">
            <FinScrollStagger className="grid gap-3 sm:grid-cols-2">
              {FIN_SECURITY_FEATURES.map((item) => (
            ...
            <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 className="text-2xl font-bold text-text-primary sm:text-3xl">
                  Ready to trade with {BRAND.name}?
                </h2>
