...
import { FIN_LIVE_METRICS, FIN_MARKET_PAIRS, type FinMarketPair } from "@/constants/fin-markets";
import {
  FinBar,
  FinColorBars,
  FinHoverLift,
...
function LiveMarketsDashboard({ pairs }: { pairs: FinMarketPair[] }) {
          ...
          <FinStaggerItem variant="scale">
            <FinHoverLift className="fin-card p-4 sm:p-6">
              <div className="mb-4 flex items-center justify-between gap-2">
                ...
                <Link to="/world-economy" className="text-xs font-medium text-text-tertiary hover:text-text-primary sm:text-sm">
                  View all
                </Link>
              </div>
...
export function FinHomePage({ pairs = FIN_MARKET_PAIRS }: { pairs?: FinMarketPair[] }) {
  return (
